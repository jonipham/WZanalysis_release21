...
class AMI_Entry(object):
    ...
    def getEvents(self, tag, data_type="AOD"):
        if not self.hasDataType(data_type):
            print "WARNING: Unkown data format %s " % (data_type)
            return 0
        try:
    ...
    def tags(self, data_type="AOD"):
        if not self.hasDataType(data_type):
            print "WARNING: Unknown data type %s" % (data_type)
            return []
        return [T for T in self.__datasets[data_type].iterkeys()]


class AMImcEntry(AMI_Entry):
    def __init__(self, dsid=1, xsec=-1., physics_name="", campaign="mc16_13TeV"):
    ...
    def nEvents(self, tag, data_type="AOD", isAFII=False):
        ...
        print "WARNING: Unknown tag %s for sample %s (%d)" % (tag, self.name(), self.dsid())
        return 0

    def getTags(self, data_type="AOD", filter_full=False, filter_fast=False):
        """filter means to drop the sim flavour from the list"""
        if not self.hasDataType(data_type):
            print "WARNING: Unkown data format %s for sample %s (%d)" % (data_type, self.name(), self.dsid())
            return []
        List = []
...
class AMIDataBase(object):
    ...
    def getMCDataSets(self, channels=[], campaign="mc16_13TeV", derivations=[]):
        ...
        print "<AMIDataBase> INFO: going to ask AMI about %d different things" % (len(channels_to_use))
        prompt = max(int(len(channels_to_use) / 10), 2)
        for i, mc in enumerate(channels_to_use):
            if i % prompt == 0:
                print "<AMIDataBase> INFO: %d/%d stubbed AMI :-P" % (i, len(channels_to_use))
            # AMI query
            ...
            for amiDS in DSIDS:
                ...
                if not ami_entry:
                    ...
                    try:
                        ...
                        print "<AMIDataBase> WARNING: No x-section found for %s (%i) in AMI" % (physics_name, DS)
                        xS = 1.
                    ami_entry = AMImcEntry(dsid=DS, xsec=xS, physics_name=physics_name, campaign=campaign)
                    self.__mc_channels.append(ami_entry)
                ds_type = amiDS["type"]
                tag = self.__getDSTag(amiDS['ldn'])
                nevents = int(amiDS['events'])
                ami_entry.addDataset(data_type=ds_type, tag=tag, events=nevents, status="")
        return True
